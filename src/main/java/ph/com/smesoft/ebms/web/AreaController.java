...
public class AreaController {
	...
	void populateForm(Model uiModel, Area area) {
    	uiModel.addAttribute("area", area);
        uiModel.addAttribute("streets", streetService.findAllStreets());
    }
	/*String roomNumber;
	...
	*/
	...
	public String listofPersons(@ModelAttribute("SearchCriteria") SearchForm searchForm,
			...
			if(search != null && !search.isEmpty() && !search.trim().isEmpty()){
				uiModel.addAttribute("areas", areaService.findAllAreasBySearch(searchForm.getSearchString()));
				return "areas/list";
			}
			else{
				uiModel.addAttribute("page", (page == null) ? "1" : page.toString());
