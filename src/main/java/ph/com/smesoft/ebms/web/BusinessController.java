...
public class BusinessController {
    ...
    public String create(@Valid Business business, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        ...
        System.out.print(businessService.checkIfBusinessExist(business.getBusinessName()));
        if(businessService.checkIfBusinessExist(business.getBusinessName()) > 0){
        	bindingResult.reject("business", "Already Exist");
        	 return "businesses/create";
        }
        uiModel.asMap().clear();
    ...
    public String list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
        if (page != null || size != null) {
            ...
            float nrOfPages = (float) businessService.countAllBusinesss() / sizeNo;
    ...
    public String update(@Valid Business business, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        if (bindingResult.hasErrors()) {
            populateEditForm(uiModel, business);
    ...
    public String updateForm(@PathVariable("id") Long id, Model uiModel) {
        populateEditForm(uiModel, businessService.findBusiness(id));
        return "businesses/update";
    }
    ...
    public ResponseEntity<String> createFromJson(@RequestBody String json, UriComponentsBuilder uriBuilder) {
        ...
        try {
            ...
            headers.add("Business",uriBuilder.path(a.value()[0]+"/"+business.getId().toString()).build().toUriString());
    ...
    public ResponseEntity<String> createFromJsonArray(@RequestBody String json) {
        ...
        headers.add("Content-Type", "application/json");
        try {
            for (Business business: Business.fromJsonArrayToBusinesss(json)) {
            	businessService.saveBusiness(business);
            }
            return new ResponseEntity<String>(headers, HttpStatus.CREATED);
        } catch (Exception e) {
    ...
    public ResponseEntity<String> updateFromJson(@RequestBody String json, @PathVariable("id") Long id) {
        ...
        try {
            Business business = Business.fromJsonToBusiness(json);
            business.setId(id);
            if (businessService.updateBusiness(business) == null) {
                return new ResponseEntity<String>(headers, HttpStatus.NOT_FOUND);
            }
...
/*
	...
	public String listofCity(@ModelAttribute("SearchCriteria") SearchForm searchForm, Model uiModel) {
		uiModel.addAttribute("businesses", businessService.findBusinessbyBusinessNumber(searchForm.getSearchString()));
		return "businesses/list";
	}
}
